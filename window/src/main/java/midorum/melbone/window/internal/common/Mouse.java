...
public class Mouse {
    ...
    public void clickAtPoint(final PointFloat point) throws InterruptedException {
        mouse.move(point);
        delay.sleep(mouseClickDelay, TimeUnit.MILLISECONDS);
        mouse.move(point).leftClick();
    }

    public void clickAtPoint(final float x, final float y) throws InterruptedException {
        mouse.move(x, y);
        delay.sleep(mouseClickDelay, TimeUnit.MILLISECONDS);
